...
public class Mold_OnOffController : BaseController
{
    private readonly Mold_OnOffApp _app;

    public Mold_OnOffController(IAuth auth, Mold_OnOffApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Mold_OnOff>) ?? Enumerable.Empty<Mold_OnOff>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Mold_OnOff");
        return File(bytes, "application/vnd.ms-excel", "Mold_OnOff.xls");
    }
}