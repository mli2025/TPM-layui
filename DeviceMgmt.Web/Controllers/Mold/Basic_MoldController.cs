...
public class Basic_MoldController : BaseController
{
    private readonly Basic_MoldApp _app;

    public Basic_MoldController(IAuth auth, Basic_MoldApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Basic_Mold>) ?? Enumerable.Empty<Basic_Mold>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Basic_Mold");
        return File(bytes, "application/vnd.ms-excel", "Basic_Mold.xls");
    }
}