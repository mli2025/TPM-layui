...
public class Mold_TheTemplateSubController : BaseController
{
    private readonly Mold_TheTemplateSubApp _app;

    public Mold_TheTemplateSubController(IAuth auth, Mold_TheTemplateSubApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Mold_TheTemplateSub>) ?? Enumerable.Empty<Mold_TheTemplateSub>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Mold_TheTemplateSub");
        return File(bytes, "application/vnd.ms-excel", "Mold_TheTemplateSub.xls");
    }
}