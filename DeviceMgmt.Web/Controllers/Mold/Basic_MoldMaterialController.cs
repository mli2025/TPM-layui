...
public class Basic_MoldMaterialController : BaseController
{
    private readonly Basic_MoldMaterialApp _app;

    public Basic_MoldMaterialController(IAuth auth, Basic_MoldMaterialApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Basic_MoldMaterial>) ?? Enumerable.Empty<Basic_MoldMaterial>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Basic_MoldMaterial");
        return File(bytes, "application/vnd.ms-excel", "Basic_MoldMaterial.xls");
    }
}