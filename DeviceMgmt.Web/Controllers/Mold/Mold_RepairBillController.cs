...
public class Mold_RepairBillController : BaseController
{
    private readonly Mold_RepairBillApp _app;

    public Mold_RepairBillController(IAuth auth, Mold_RepairBillApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Mold_RepairBill>) ?? Enumerable.Empty<Mold_RepairBill>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Mold_RepairBill");
        return File(bytes, "application/vnd.ms-excel", "Mold_RepairBill.xls");
    }
}