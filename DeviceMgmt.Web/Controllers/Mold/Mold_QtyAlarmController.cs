...
public class Mold_QtyAlarmController : BaseController
{
    private readonly Mold_QtyAlarmApp _app;

    public Mold_QtyAlarmController(IAuth auth, Mold_QtyAlarmApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<v_MoldQtyAlarm>) ?? Enumerable.Empty<v_MoldQtyAlarm>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Mold_QtyAlarm");
        return File(bytes, "application/vnd.ms-excel", "Mold_QtyAlarm.xls");
    }
}