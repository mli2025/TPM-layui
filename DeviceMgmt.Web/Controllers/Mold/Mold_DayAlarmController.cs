...
public class Mold_DayAlarmController : BaseController
{
    private readonly Mold_DayAlarmApp _app;

    public Mold_DayAlarmController(IAuth auth, Mold_DayAlarmApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<v_MoldDayAlarm>) ?? Enumerable.Empty<v_MoldDayAlarm>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Mold_DayAlarm");
        return File(bytes, "application/vnd.ms-excel", "Mold_DayAlarm.xls");
    }
}