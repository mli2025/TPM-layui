...
public class WuzikuController : BaseController
{
    private readonly WuzikuApp _app;

    public WuzikuController(IAuth auth, WuzikuApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Wuziku>) ?? Enumerable.Empty<Wuziku>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Wuziku");
        return File(bytes, "application/vnd.ms-excel", "Wuziku.xls");
    }
}