...
public class Spare_InvoiceDataController : BaseController
{
    private readonly Spare_InvoiceDataApp _app;

    public Spare_InvoiceDataController(IAuth auth, Spare_InvoiceDataApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Spare_InvoiceData>) ?? Enumerable.Empty<Spare_InvoiceData>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Spare_InvoiceData");
        return File(bytes, "application/vnd.ms-excel", "Spare_InvoiceData.xls");
    }
}