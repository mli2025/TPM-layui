...
public class Facility_BillSubController : BaseController
{
    private readonly Facility_BillSubApp _app;

    public Facility_BillSubController(IAuth auth, Facility_BillSubApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Facility_BillSub>) ?? Enumerable.Empty<Facility_BillSub>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_BillSub");
        return File(bytes, "application/vnd.ms-excel", "Facility_BillSub.xls");
    }
}