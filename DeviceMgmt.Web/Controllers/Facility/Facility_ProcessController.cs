...
public class Facility_ProcessController : BaseController
{
    private readonly Facility_ProcessApp _app;

    public Facility_ProcessController(IAuth auth, Facility_ProcessApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Facility_Process>) ?? Enumerable.Empty<Facility_Process>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_Process");
        return File(bytes, "application/vnd.ms-excel", "Facility_Process.xls");
    }
}