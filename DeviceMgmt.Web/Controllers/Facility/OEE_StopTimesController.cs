...
public class OEE_StopTimesController : BaseController
{
    private readonly OEE_StopTimesApp _app;

    public OEE_StopTimesController(IAuth auth, OEE_StopTimesApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<OEE_StopTimes>) ?? Enumerable.Empty<OEE_StopTimes>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "OEE_StopTimes");
        return File(bytes, "application/vnd.ms-excel", "OEE_StopTimes.xls");
    }
}