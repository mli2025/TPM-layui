...
public class Facility_Status_HistoryController : BaseController
{
    private readonly Facility_Status_HistoryApp _app;

    public Facility_Status_HistoryController(IAuth auth, Facility_Status_HistoryApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Facility_Status_History>) ?? Enumerable.Empty<Facility_Status_History>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_Status_History");
        return File(bytes, "application/vnd.ms-excel", "Facility_Status_History.xls");
    }
}