...
public class Facility_RepairBillSubController : BaseController
{
    private readonly Facility_RepairBillSubApp _app;

    public Facility_RepairBillSubController(IAuth auth, Facility_RepairBillSubApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Facility_RepairBillSub>) ?? Enumerable.Empty<Facility_RepairBillSub>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_RepairBillSub");
        return File(bytes, "application/vnd.ms-excel", "Facility_RepairBillSub.xls");
    }
}