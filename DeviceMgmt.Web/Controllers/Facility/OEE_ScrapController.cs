...
public class OEE_ScrapController : BaseController
{
    private readonly OEE_ScrapApp _app;

    public OEE_ScrapController(IAuth auth, OEE_ScrapApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<OEE_Scrap>) ?? Enumerable.Empty<OEE_Scrap>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "OEE_Scrap");
        return File(bytes, "application/vnd.ms-excel", "OEE_Scrap.xls");
    }
}