...
public class Production_BarcodeSMTController : BaseController
{
    private readonly Production_BarcodeSMTApp _app;

    public Production_BarcodeSMTController(IAuth auth, Production_BarcodeSMTApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<V_Production_BarcodeSMT>) ?? Enumerable.Empty<V_Production_BarcodeSMT>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Production_BarcodeSMT");
        return File(bytes, "application/vnd.ms-excel", "Production_BarcodeSMT.xls");
    }
}