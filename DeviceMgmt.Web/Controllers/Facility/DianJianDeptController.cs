...
public class DianJianDeptController : BaseController
{
    private readonly DianJianDeptApp _app;

    public DianJianDeptController(IAuth auth, DianJianDeptApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<v_dianjiandept>) ?? Enumerable.Empty<v_dianjiandept>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "DianJianDept");
        return File(bytes, "application/vnd.ms-excel", "DianJianDept.xls");
    }
}