...
public class Facility_DATAController : BaseController
{
    private readonly Facility_DATAApp _app;

    public Facility_DATAController(IAuth auth, Facility_DATAApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Facility_DATA>) ?? Enumerable.Empty<Facility_DATA>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_DATA");
        return File(bytes, "application/vnd.ms-excel", "Facility_DATA.xls");
    }
}