...
public class Facility_RepairEmpController : BaseController
{
    private readonly Facility_RepairEmpApp _app;

    public Facility_RepairEmpController(IAuth auth, Facility_RepairEmpApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Facility_RepairEmp>) ?? Enumerable.Empty<Facility_RepairEmp>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_RepairEmp");
        return File(bytes, "application/vnd.ms-excel", "Facility_RepairEmp.xls");
    }
}