...
public class OEE_TotalTimesController : BaseController
{
    private readonly OEE_TotalTimesApp _app;

    public OEE_TotalTimesController(IAuth auth, OEE_TotalTimesApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<OEE_TotalTimes>) ?? Enumerable.Empty<OEE_TotalTimes>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "OEE_TotalTimes");
        return File(bytes, "application/vnd.ms-excel", "OEE_TotalTimes.xls");
    }
}