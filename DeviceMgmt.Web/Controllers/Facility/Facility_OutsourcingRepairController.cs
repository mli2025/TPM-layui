...
public class Facility_OutsourcingRepairController : BaseController
{
    private readonly Facility_OutsourcingRepairApp _app;

    public Facility_OutsourcingRepairController(IAuth auth, Facility_OutsourcingRepairApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Facility_OutsourcingRepair>) ?? Enumerable.Empty<Facility_OutsourcingRepair>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_OutsourcingRepair");
        return File(bytes, "application/vnd.ms-excel", "Facility_OutsourcingRepair.xls");
    }
}