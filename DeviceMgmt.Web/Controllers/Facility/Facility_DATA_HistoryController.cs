...
public class Facility_DATA_HistoryController : BaseController
{
    private readonly Facility_DATA_HistoryApp _app;

    public Facility_DATA_HistoryController(IAuth auth, Facility_DATA_HistoryApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<Facility_DATA_History>) ?? Enumerable.Empty<Facility_DATA_History>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_DATA_History");
        return File(bytes, "application/vnd.ms-excel", "Facility_DATA_History.xls");
    }
}