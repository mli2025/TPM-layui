...
public class OEE_RateController : BaseController
{
    private readonly OEE_RateApp _app;

    public OEE_RateController(IAuth auth, OEE_RateApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<OEE_Rate>) ?? Enumerable.Empty<OEE_Rate>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "OEE_Rate");
        return File(bytes, "application/vnd.ms-excel", "OEE_Rate.xls");
    }
}