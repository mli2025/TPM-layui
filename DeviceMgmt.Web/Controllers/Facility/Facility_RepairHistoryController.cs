...
public class Facility_RepairHistoryController : BaseController
{
    private readonly Facility_RepairHistoryApp _app;

    public Facility_RepairHistoryController(IAuth auth, Facility_RepairHistoryApp app) : base(auth)
    {
        _app = app;
    }
    ...
    public IActionResult ExportExcel([FromForm] PageReq req)
    {
        ...
        var rows = (pageData.data as IEnumerable<v_Facility_RepairHistory>) ?? Enumerable.Empty<v_Facility_RepairHistory>();
        var dt = NPOIHelper.LINQToDataTable(rows);
        var bytes = NPOIHelper.ExportToBytes(dt, "Facility_RepairHistory");
        return File(bytes, "application/vnd.ms-excel", "Facility_RepairHistory.xls");
    }
}