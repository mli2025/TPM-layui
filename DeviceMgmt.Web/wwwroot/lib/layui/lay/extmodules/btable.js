layui.define(['element', 'common', 'paging', 'form'], function (exports) {
    ...
    BTable.prototype.render = function () {
        ...
        if (_config.checkbox && !_config.singleSelect) {
            th = '<th style="width:28px;"><input type="checkbox" lay-filter="allselector" lay-skin="primary" /></th><th style="width:38px;">序号</th>' + th;
        } else if (_config.checkbox) {
            th = '<th style="width:28px;">#</th><th style="width:38px;">序号</th>' + th;
        } else {
            th = '<th style="width:38px;">序号</th>' + th;
        }
                    ...
                    if (columns[i].format) {
                        $('#' + dataId).find('tr').each(function () {
                            ...
                            for (var j = 0; j < data.length; j++) {
                                for (var ke in data[j]) {
                                    if (data[j][_config.field] == id) {
                                        obj = data[j];
                                        break;
                                    }
                                }
