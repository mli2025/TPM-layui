layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            globalTabIdIndex++;
            //console.log(data)
            let targetHref = data.href;
            if (targetHref.indexOf("?") > -1)
                targetHref += "&moduleId=" + data.id;
            else
                targetHref += "?moduleId=" + data.id;
            var content = '<iframe src="' + targetHref + '" data-id="' + globalTabIdIndex + '"></iframe>';
            var title = '';
        ...
        if (_config.contextMenu) {
            ...
            ELEM.titleBox.find('li').on('contextmenu', function (e) {
                ...
                var ul = '<ul>';
                ul += '<li data-target="refresh" title="' + lang.tabReloadTitle + '"><i class="fa fa-refresh" aria-hidden="true"></i> ' + lang.tabReload + '</li>';
                ul += '<li data-target="closeCurrent" title="' + lang.tabCloseThisTitle + '"><i class="fa fa-close" aria-hidden="true"></i> ' + lang.tabCloseThis + '</li>';
                ul += '<li data-target="closeOther" title="' + lang.tabCloseOtherTitle + '"><i class="fa fa-window-close-o" aria-hidden="true"></i> ' + lang.tabCloseOther + '</li>';
                ul += '<li data-target="closeAll" title="' + lang.tabCloseAllTitle + '"><i class="fa fa-window-close-o" aria-hidden="true"></i> ' + lang.tabCloseAll + '</li>';
                ul += '</ul>';
                div.innerHTML = ul;
