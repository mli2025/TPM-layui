layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    */
    tableSelect.prototype.render = function (opt) {
        ...
        let conditionRes = opt.extCondition();
        //默认设置
        opt.searchKey = opt.searchKey || 'key';
        opt.searchQuery = opt.searchQuery || 'query';
        opt.searchPlaceholder = opt.searchPlaceholder || lang.tableSelectSearchPlaceholder;
        opt.extCondition = opt.extCondition || [];
        ...
        elem.off('click').on('click', function (e) {
            e.stopPropagation();
            conditionRes = opt.extCondition();
            if ($('div.tableSelect').length >= 1) {
                return false;
            }
            let extControl = "";
            
            for (let i = 0; i < conditionRes.length; i++) {
                let conditional = "=";
                if (typeof conditionRes[i].conditional != "undefined" && conditionRes[i].conditional != null && conditionRes[i].conditional != "")
                    conditional = conditionRes[i].conditional;
                extControl += "<input type='hidden' name='searchParam[" + i + "][field]' value='" + conditionRes[i].field + "' />";
                extControl += "<input type='hidden' name='searchParam[" + i + "][conditional]' value='" + conditional + "' />";
                extControl += "<input type='hidden' name='searchParam[" + i + "][value]' value='" + conditionRes[i].value + "' />";
            }

            var t = elem.offset().top + elem.outerHeight() + "px";
            var l = elem.offset().left + "px";
            var tableName = "tableSelect_table_" + new Date().getTime();
            var tableBox = '<div class="tableSelect layui-anim layui-anim-upbit" style="left:' + l + ';top:' + t + ';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-index:69891016;margin: 5px 0;border-radius: 2px;min-width:530px;">';
            tableBox += '<div class="tableSelectBar">';
            tableBox += '<form class="layui-form" action="" style="display:inline-block;">';
            tableBox += '<input type="hidden" name="' + opt.searchKey + '" /><input style="display:inline-block;width:190px;height:30px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;" type="text" name="' + opt.searchQuery + '" placeholder="' + opt.searchPlaceholder + '" autocomplete="off" class="layui-input searchInput"><button class="layui-btn layui-btn-sm layui-btn-primary tableSelect_btn_search" lay-submit lay-filter="tableSelect_btn_search">' + lang.tableSelectSearch+'</button>';
            //tableBox += extControl;
            tableBox += '</form>';
            tableBox += '<button style="float:right;" class="layui-btn layui-btn-sm tableSelect_btn_select">' + lang.tableSelectChoose+'<span></span></button>';
            tableBox += '</div>';
            ...
            opt.table.size = "sm";
            opt.table.method = "POST";
            let conditionExt = [];
            conditionExt.searchParam = [];
            for (let i = 0; i < conditionRes.length; i++) {
                let conditional = "=";
                if (typeof conditionRes[i].conditional != "undefined" && conditionRes[i].conditional != null && conditionRes[i].conditional != "")
                    conditional = conditionRes[i].conditional;
                conditionExt.searchParam.push({ field: conditionRes[i].field, conditional: conditional, value: conditionRes[i].value});
                //conditionExt.searchParam[i].field = conditionRes[i].field;
                //conditionExt.searchParam[i].conditional = conditional;
                //conditionExt.searchParam[i].value = conditionRes[i].value;
            }
            //console.log(conditionExt)
            if (typeof opt.table.where == "undefined")
                opt.table.where = conditionExt;
            else if (typeof opt.table.where.searchParam == "undefined") {
                opt.table.where.searchParam = conditionExt.searchParam;
            }
            else {
                opt.table.where.searchParam = (conditionExt.searchParam)
            }
            ...
            form.on('submit(tableSelect_btn_search)', function (data) {
                console.log(tableSelect_table)
                $.extend(true,tableSelect_table.config.where, data.field);
                tableSelect_table.reload({
                    where: tableSelect_table.config.where,
                    page: {
                        curr: 1
                    }
                });
        ...
        $(document).on("click", ".tableSelect .layui-table-body table.layui-table tbody tr", function () {
        ...
        $(document).on("click", ".tableSelect td div.laytable-cell-checkbox div.layui-form-checkbox", function (e) {
            e.stopPropagation();
        });
        ...
        $(document).on("click", ".tableSelect td div.laytable-cell-radio div.layui-form-radio", function (e) {
            e.stopPropagation();
        });
    }
