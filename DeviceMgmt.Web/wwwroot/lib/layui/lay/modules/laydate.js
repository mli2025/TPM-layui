...
; !function () {
    ...
    var isLayui = window.layui && layui.define, ready = {
        ...
        //字符常量
        , MOD_NAME = 'laydate', ELEM = '.layui-laydate', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'laydate-disabled', TIPS_OUT = lang.laydateTIPSOUT, LIMIT_YEAR = [100, 200000]
    ...
    */
    ...
    Class.prototype.render = function () {
        ...
        lay(divFooter).html(function () {
            ...
            if (options.type === 'datetime') {
                html.push('<span lay-type="datetime" class="laydate-btns-time">' + langi.timeTips + '</span>');
            }
            lay.each(options.btns, function (i, item) {
                var title = langi.tools[item] || 'btn';
                if (options.range && item === 'now') return;
                if (isStatic && item === 'clear') title = options.lang === 'cn' ? lang.laydateReset : 'Reset';
    ...
    Class.prototype.calendar = function (value) {
        ...
        if (dateTime.year < LIMIT_YEAR[0]) dateTime.year = LIMIT_YEAR[0], that.hint(lang.laydateMinYear + LIMIT_YEAR[0]);
        if (dateTime.year > LIMIT_YEAR[1]) dateTime.year = LIMIT_YEAR[1], that.hint(lang.laydateMaxYear + LIMIT_YEAR[1]);
        ...
        if (options.lang === 'cn') {
            ...
            lay(elemYM[0]).attr('lay-type', 'month').html(langi.month[dateTime.month]);
    ...
    Class.prototype.list = function (type, index) {
        ...
        if (type === 'year') { //年列表
            ...
            lay.each(new Array(12), function (i) {
                ...
                li.innerHTML = langi.month[i] + (isCN ? '' : '');
                ul.appendChild(li);
            ...
            lay.each([24, 60, 60], function (i, item) {
                var li = lay.elem('li'), childUL = ['<p>' + langi.time[i] + '</p><ol>'];
        ...
        if (type === 'year' || type === 'month') {
            ...
            scroll()
            span.innerHTML = options.range ? [langi.startTime, langi.endTime][index] : langi.timeTips
    ...
    Class.prototype.tool = function (btn, type) {
        var that = this
            ...
            , active = {
                ...
                , confirm: function () {
                    if (options.range) {
                        if (!that.endDate) return that.hint(lang.laydateRangeTip);
                        if (lay(btn).hasClass(DISABLED)) return that.hint(
                            options.type === 'time' ? lang.laydateTimeTIPSOUT : TIPS_OUT
                        );
                    } else {
                        if (lay(btn).hasClass(DISABLED)) return that.hint(lang.laydateOutOfRange);
                    }
                    that.done();
                    that.setValue(that.parse()).remove()
                }
