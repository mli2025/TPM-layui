layui.define(['layer', 'carousel'], function (exports) {
    ...
    var renderDom = function (elem, stepItems, postion) {
        ...
        for (var i = 0; i < stepItems.length; i++) {
            ...
            if (i == postion) {
                stepDiv += '<div class="step-item-head step-item-head-active step' + number + '"><i class="layui-icon">' + number + '</i></div>';
            } else if (i < postion) {
                stepDiv += '<div class="step-item-head step' + number +'"><i class="layui-icon layui-icon-ok"></i></div>';
            } else {
                stepDiv += '<div class="step-item-head step' + number +'"><i class="layui-icon">' + number + '</i></div>';
            }
    ...
    function getCurrent() {
        let step = $('.step-item-head')
        let reservIndex = 0
        step.each((index, item) => {
            if ($(item).hasClass('step-item-head-active')) {
                reservIndex = index
            }
        })
        return reservIndex
    ...
    var step = {
        ...
        jump: function (index) {
            ...
            $('.step' + reservIndex).removeClass('step-item-head-active')
            $('.step' + index).addClass('step-item-head-active')
        },
        // 下一步
        next: function (elem) {
