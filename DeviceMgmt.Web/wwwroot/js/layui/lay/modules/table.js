...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util', 'laydate'], function (exports) {
    ...
    let NumberToLocalString = function (s, n) {
    ...
    Class.prototype.render = function () {
        ...
        let localCols = layui.data("tableCol")[options.elem.selector + window.location.pathname + window.location.search];
        if (options.toolbar && options.defaultToolbar.indexOf("filter") > -1)
            $.extend(true, options.cols, localCols);
    ...
    Class.prototype.renderToolbar = function () {
        ...
        var layout = {
            filter: {
                title: lang.tableLayoutFilter
                , layEvent: 'LAYTABLE_COLS'
                , icon: 'layui-icon-cols'
            }
            , exports: {
                title: lang.tableLayoutExports
                , layEvent: 'LAYTABLE_EXPORT'
                , icon: 'layui-icon-export'
            }
            , print: {
                title: lang.tableLayoutPrint
                ...
                , success: function (res) {
                    ...
                    if (res[response.statusName] != response.statusCode) {
                        ...
                        that.layMain.html('<div class="' + NONE + '">' + (
                            ...
                            (lang.tableResponseStatusErr + '(' + response.statusName + '): ' + response.statusCode)
                        ) + '</div>');
                    } else {
                    ...
                    if (typeof buttons != "undefined" && buttons != null)
                        for (let z = 0; z < buttons.length; z++) {
                            if (buttons[z].Script != "") {
                                $("*[powerid='" + buttons[z].DomId + "']").removeClass("layui-hide");
                            }
                        }
                    typeof options.done === 'function' && options.done(res, curr, res[response.countName]);
                }
                , error: function (e, m) {
                    that.layMain.html('<div class="' + NONE + '">' + lang.tableResponseException + ':' + m + '</div>');
                    that.renderForm();
                    that.setColsWidth();
                }
            });
                ...
                layui.each(data, function (i1, item1) {
                    ...
                    that.eachCols(function (i3, item3) {
                        var field = item3.field || i3
                            ...
                            , orivalue = item1[field]
                            , content = item1[field];

                        if (content === undefined || content === null) content = '';
                        if (item3.colGroup) return;
                        if (!isNaN(content)) {
                            content = item3.parseLocal && (item3.parseLocal == true || item3.parseLocal == "cell") ? NumberToLocalString(content, item3.decimal || 2) : content;
                        }
                        //td内容
                        var td = ['<td data-field="' + field + '" data-key="' + key + '" ' + function () { //追加各种属性
                            var attr = [];
                            if (orivalue) attr.push('data-value="' + orivalue + '"'); //是否允许单元格编辑
                            ...
                            if (item3.selectdata) attr.push('data-selectdata="' + item3.selectdata + '"'); //自定义样式
                            if (item3.fdisplay) attr.push('data-fdisplay="' + item3.fdisplay + '"'); //自定义样式
                            if (item3.foreigntable) attr.push('data-foreigntable="' + item3.foreigntable + '"'); //自定义样式
                            if (item3.foreignfit) {
                                if (typeof item3.foreignfit === 'function') {
                                    attr.push('data-foreignfit="' + item3.foreignfit(item1) + '"');
                                } else {
                                    attr.push('data-foreignfit="' + item3.foreignfit + '"');
                                }
                            }//自定义样式
                            if (item3.fvalue) attr.push('data-fvalue="' + item3.fvalue + '"'); //自定义样式
        ...
        that.eachCols(function (i3, item3) {
            ...
            var td = ['<td data-field="' + field + '" data-key="' + options.index + '-' + item3.key + '" ' + function () {
                ...
                , '<div class="layui-table-cell laytable-cell-' + function () { //返回对应的CSS类标识
                    ...
                    return item3.totalRow ? (item3.parseLocal && (item3.parseLocal == true || item3.parseLocal == "total") ? NumberToLocalString(totalNums[field], item3.decimal || 2) : parseFloat(totalNums[field]).toFixed(item3.decimal || 2) || text) : text;
                }()
                , '</div></td>'].join('');
    ...
    Class.prototype.events = function () {
        ...
        that.layTool.on('click', '*[lay-event]', function (e) {
            ...
            switch (events) {
                case 'LAYTABLE_COLS': //筛选列
                    openPanel({
                        list: function () {
                            ...
                            lis.push('<li style="text-align:center" title="' + lang.tableClearLocalSetTitle + '"><button type="button" class="layui-btn layui-btn-xs clearLocal">' + lang.tableClearLocalSet + '</button></li>')
                            return lis.join('');
                        }()
                        , done: function () {
                            form.on('checkbox(LAY_TABLE_TOOL_COLS)', function (obj) {
                                ...
                                layui.data("tableCol", {
                                    key: options.elem.selector + window.location.pathname + window.location.search,
                                    value: options.cols
                                })
                ...
                case 'LAYTABLE_EXPORT': //导出
                    if (device.ie) {
                        layer.tips(lang.tableExportNotIE, this, {
                            tips: 3
                        })
                    } else {
                        openPanel({
                            list: function () {
                                return [
                                    '<li data-type="csv">' + lang.tableExportToCsv + '</li>'
                                    , '<li data-type="xls">' + lang.tableExportToExcel + '</li>'
                                ].join('')
                            }()
                ...
                case 'LAYTABLE_PRINT': //打印
                    var printWin = window.open(lang.tablePrintWndow, '_blank')
                        , style = ['<style>'
        ...
        th.on('mousemove', function (e) {
            ...
            let fieldvs = $(this).data('field');
            let filterCol = function (item) {
                if (item.field == fieldvs)
                    return true;
                return false;
            }
            //console.log(options.cols[0]);
            //console.log($(this).outerWidth())
            let col = options.cols[0].find(filterCol);
            if (typeof col != "undefined")
                options.cols[0].find(filterCol).width = $(this).outerWidth();

            //options.cols[0][]
            //console.log(options.elem.selector);
            layui.data("tableCol", {
                key: options.elem.selector + window.location.pathname + window.location.search,
                value: options.cols
            })
        ...
        that.layBody.on('change', '.' + ELEM_EDIT, function () {
            ...
            othis.parent().data('value', this.value);

            if (editType != "select2")
                othis.remove();
            else {
                if (othis.parent().find(".select2").length == 0) {
                    //othis.remove();
                }
                ...
                //    //$(input).css({"width": 0});
                //    $(othis).parent().children('.select2').remove();
                //});
            }
        });
        ...
        that.layBody.on('click', 'td', function (e) {
            ...
            if (editType == "text") {
                ...
                input[0].value = othis.data('value') || elemCell.text();
            ...
            if (editType == "number") {
                var input = $('<input type="number" class="layui-input ' + ELEM_EDIT + '">');
                input[0].value = othis.data('value') || elemCell.text();
                othis.find('.' + ELEM_EDIT)[0] || othis.append(input);
                input.focus();
                layui.stope(e);
            ...
            if (editType == "date") {
                let curValue = othis.data('value') || elemCell.text();
                layui.laydate.render({
                    elem: othis[0].children[0],
                    value: curValue.split(' ')[0],
                    zIndex: 999999999,
                    show: true,
                    done: function (value, date, endDate) {
                        $(othis).data('content', value);
                        $(othis).data('value', value);
            ...
            if (editType == "select") {
                var input = $('<select style="display:block" class="layui-select ' + ELEM_EDIT + '"></select>');
                let selectData = othis.data('selectdata');
                let dataOption = eval(selectData);
                console.log(selectData)
                console.log(dataOption)
                layui.each(dataOption, function (index, item) {
                    $(input).append('<option value="' + item.value + '">' + item.text + '</option>');
                })
                $(input).val(othis.data('value') || elemCell.text())
            ...
            if (editType == "select2") {
                ...
                if (select2clicked) {
                    var input = $('<select style="display:block;width:100%" class="' + ELEM_EDIT + '"></select>');
                    let ftable = othis.data('foreigntable');
                    let fdisplay = othis.data('fdisplay');
                    let fvalue = othis.data('fvalue');
                    let foreignfit = othis.data('foreignfit');
                    $(input).val(othis.data('content') || elemCell.text());
                    if (othis.find('.' + ELEM_EDIT).length == 0)
                        othis.append(input);
                    else
                        input = othis.find('.' + ELEM_EDIT)[0];
                    ...
                    if (!$(input).hasClass("select2-hidden-accessible")) {
                        ...
                        $(input).on('select2:select', function (e) {
                            ...
                            //$(input).css({"width": 0});
                            //$(othis).parent().children('.select2').remove();
                        }).select2('val', othis.data('value') || elemCell.text());
                    }
            ...
            if (editType == "callFunc") {
                layui.event.call(this, MOD_NAME, 'editCell(' + filter + ')', commonMember.call(this, {
                    value: table.cache[that.key][index][field]
                    , field: field
                    , td: othis
                    , data: table.cache[that.key][index]
                }));
